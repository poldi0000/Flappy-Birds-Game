...
const bird = {
    ...
    update: function(){
        //Flapping slowly when the game state is get ready
        this.period = state.current == state.getReady ? 10 : 5;
        //Increment frame by 1 each period
        this.frame += frames%this.period == 0 ? 1 : 0;
        //Animation frame goes from 0 to 3
...
const pipes = {
    ...
    update: function(){
        ...
        for(let i = 0; i < this.position.length; i++){
            ...
            //Bottom
            if(bird.x + bird.radius > p.x && bird.x - bird.radius < p.x + this.w &&
            bird.y + bird.radius > bottomPipeYposition && bird.y - bird.radius < bottomPipeYposition + this.h){
                    state.current = state.over;
            }
            //Moving pipes to the left
            p.x -= this.dx;
            //Deleting pipes if they go beyond canvas
            if(p.x + this.w <= 0){
                this.position.shift();
                score.value += 1;
